...
export function MultiStepEmployeeForm({ employee, departments, onSuccess }: MultiStepEmployeeFormProps) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['employees'] }); // Internal query key
      queryClient.invalidateQueries({ queryKey: ["/api/employees"] }); // Admin employee list
      if (isEditing && employee?.id) {
        queryClient.invalidateQueries({ queryKey: ["/api/employees", employee.id] }); // Specific employee
        queryClient.invalidateQueries({ queryKey: [`/api/employees/${employee.id}`] }); // Employee detail page
      }
                                ...
                                <FormItem>
                                  ...
                                  <FormControl>
                                    <Input
                                      ...
                                      className={`h-12 border rounded-xl text-sm font-medium transition-all duration-200 ${isEditing
